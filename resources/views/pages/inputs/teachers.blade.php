...
<style>
    ...
    .avatar{ width:56px; height:56px; border-radius:50%; object-fit:cover; border:1px solid #e9ecef; }
</style>
@endpush
            ...
            <div class="card glass-card border-0 shadow-sm">
                <div class="card-header bg-white" style="border-radius:22px 22px 0 0;">
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <h5 class="mb-0 fw-semibold">إضافة معلم</h5>
                    </div>
                </div>
                <div class="card-body">
                    @if(session('success'))
                    ...
                    <form method="POST" action="{{ route('inputs.teachers.store') }}" enctype="multipart/form-data" class="row g-3">
                        @csrf
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="input-group">
                                ...
                                <input type="email" name="email" class="form-control" value="{{ old('email') }}" placeholder="teacher@example.com">
                            </div>
                        </div>
...
<div class="col-md-4">
            ...
            <div class="card border-0 shadow-sm mt-3">
                <div class="card-header bg-white d-flex align-items-center gap-2">
                    ...
                    <span class="fw-semibold">المعلمون</span>
                </div>
                <div class="card-body">
                                ...
                                <tbody>
                                    @foreach($teachers as $t)
                                        <tr>
                                            <td>
                                                @if($t->avatar_path)
                                                    <img src="{{ asset('storage/'.$t->avatar_path) }}" class="avatar" alt="avatar">
                                                @else
                                            ...
                                            <td>{{ $t->department ?? '-' }}</td>
                                            <td>
                                                <span class="badge text-bg-{{ $t->status==='نشط' ? 'success' : ($t->status==='موقوف' ? 'warning' : 'secondary') }}">
                                                    {{ $t->status }}
                                                </span>
                                            </td>
                                            <td class="text-center">
                                                <a href="{{ route('inputs.teachers.edit', $t) }}" class="btn btn-sm btn-outline-primary">
                                                    <i class="bi bi-pencil-square"></i> تعديل
                                                </a>
                                                <form action="{{ route('inputs.teachers.destroy', $t) }}" method="POST" class="d-inline"
                                                      onsubmit="return confirm('تأكيد حذف المعلم؟');">
                                                    @csrf @method('DELETE')
                    ...
                    @else
                        <div class="alert alert-info mb-0">لا توجد سجلات معلمين بعد.</div>
                    @endif
                </div>
            </div>
